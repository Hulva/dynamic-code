...
import static hulva.luva.xx.demo.dynamiccode.compiler.CompilerUtil.*;
...
public class CachedCompiler implements Closeable {
  ...
  private final Map<ClassLoader, Map<String, Class<?>>> loadedClassesMap =
      Collections.synchronizedMap(new WeakHashMap<ClassLoader, Map<String, Class<?>>>());
  private final Map<ClassLoader, LuvaJavaFileManager> fileManagerMap =
      Collections.synchronizedMap(new WeakHashMap<ClassLoader, LuvaJavaFileManager>());
  ...
  public void close() {
    try {
      for (LuvaJavaFileManager fileManager : fileManagerMap.values()) {
        fileManager.close();
      }
    } catch (IOException e) {
  ...
  @NotNull
  Map<String, byte[]> compileFromJava(@NotNull String className, @NotNull String javaCode, LuvaJavaFileManager fileManager) {
  ...
  Map<String, byte[]> compileFromJava(@NotNull String className, @NotNull String javaCode, final @NotNull PrintWriter writer,
      LuvaJavaFileManager fileManager) {
  ...
  public Class<?> loadFromJava(@NotNull ClassLoader classLoader, @NotNull String className, @NotNull String javaCode,
    ...
    LuvaJavaFileManager fileManager = fileManagerMap.get(classLoader);
    if (fileManager == null) {
      StandardJavaFileManager standardJavaFileManager = s_compiler.getStandardFileManager(null, null, null);
      fileManagerMap.put(classLoader, fileManager = new LuvaJavaFileManager(standardJavaFileManager));
    }
